...
img.addEventListener("load", function() {
    ...
    if(edge || edge2) {
        ...
        paletteCopy = copyImageData(paletteCtx, paletteData);
/*        for(var px=0;px<100;px++) {
            for(var py=0;py<100;py++) {
                setPixel(paletteCopy,px,py, [255,0,0,255]);
            }
        }*/
...
function swapPixels(imgData, x,y,x1,y1, width) {
    ...
    setPixel(imgData, x1, y1, p);
    setPixel(imgData, x, y, pdest);
}
...
function setPixel(imgData, x, y, pixel) {
    var pixelIndex = (x + y*imgData.width)*4;
...
var randomness = 0;
function drawFrom(x,y,x1,y1, paintSize, erase, color) {
...
/*
    ...
    if(paletteScale && false) {
        var ss = paletteScale*2;
        var px0 = ((x - palette.width) * ss);
        var py0 = (y * ss);
        var px1 = ((x1 - palette.width) * ss);
        var py1 = (y1 * ss);
        var range = Math.ceil(Math.max(Math.abs(px1-px0),Math.abs(py1-py0)));
//        console.log(range);
        for(var i=0; i<range; i++) {
            var px = Math.round(px1 * i/range + px0 * (range-i)/range);
            var py = Math.round(py1 * i/range + py0 * (range-i)/range);
//            keyInfo.innerText = JSON.stringify([px,py]);
            var isErased = getPixel(paletteCopy,px,py)[0]===255;
            if(isErased !== erase) {
                var shouldErase = getPixel(paletteData,px,py)[0]===255;
                if(shouldErase === erase) {
                    score++;
                } else {
//                    console.log(getPixel(paletteCopy,px,py), getPixel(paletteData,px,py), erase);
                    score--;
                }
                setPixel(paletteCopy,px,py, erase ? [255,255,255,255]: [0,0,0,255]);
            }
        }
        keyInfo.innerText = "SCORE: " + score;
        paletteCtx.putImageData(paletteCopy,0,0);
    }
//    for(var i=0; i<1; i++) {
        canvasCtx.lineWidth =10;
        canvasCtx.strokeStyle = !both && erase ? "#CCC0B6" : color;
        canvasCtx.moveTo(
...
//        if(keyboard[player.act]) {
            drawFrom(player.x, player.y, nextX, nextY, paintSize, !keyboard[player.act], player.fillColor);
  //      }
        player.x = nextX;
        player.y = nextY;
    });
...
//                console.log(player.goal);
            ...
            if(player.goal) {
                ...
                if(dist > 0) {
                    ...
                    var ddy = dy/dist * d;
//                    if(player.pen) {
                        drawFrom(player.x,player.y,player.x + ddx, player.y + ddy,paintSize, !player.pen, player.fillColor);
//                    }
                    player.x += ddx;
                    player.y += ddy;
                } else {
            ...
            var dist = Math.sqrt(dx*dx+dy*dy);
            var d= Math.min(dist, point.down?20:40);
            if(dist > 0) {
                var ddx = dx/dist * d;
                var ddy = dy/dist * d;
                drawFrom(player.x,player.y,player.x + ddx, player.y + ddy,paintSize, !point.down, player.fillColor);
                player.x += ddx;
                player.y += ddy;
            }
        }
